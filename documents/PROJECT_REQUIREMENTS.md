...
1. **Two separate buttons** (not toggle on same button)
2. **SD card file access** (not just GPIO)
3. **Audio playback** (new component)
4. **Assembly for file parsing** (not just GPIO control)
5. **10-second timer** (instead of 1-second)
6. **Same button cancels** (not separate cancel button)
